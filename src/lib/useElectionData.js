...
export function useElectionData() {
  ...
  const loadAll = useCallback(async () => {
    try {
      ...
      const { data: parts, error: e2 } = await sb.from('parties').select('*').order('sort_order');
      if (e2) throw e2;
      ...
      allVotes.forEach(r => {
        ...
        voteMap[r.constituency_id][r.party_code] ??= {};
        voteMap[r.constituency_id][r.party_code][r.round] = { votes: r.votes, estimated: r.is_estimated, entryMode: r.entry_mode || 'total' };
      });

      const candMap = {};
      allCands.forEach(r => {
        candMap[r.constituency_id] ??= {};
        candMap[r.constituency_id][r.party_code] = { name: r.candidate_name, photo: r.photo_url };
      });
